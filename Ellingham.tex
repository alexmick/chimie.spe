...
\end{enumerate}
		...
		L'approximation d'\bsc{Ellingham} consiste à considérer les enthalpies libres et entropies libres de réactions comme indépendantes de la température (mais elles restent sensibles aux changements d'état). Ainsi, $\Delta rG^\circ = a+bT \mbox{ avec } a= \Delta rH^\circ=\mbox{cste et } b = \Delta rS^\circ=\mbox{cste}$. A l'exception d'\chemfig{O_2} tous les constituants sont dans leurs états standards. \\ 
		Considérons l'équilibre \schemestart \chemfig{Zn_{(s)}} \arrow{<=>} \chemfig{Zn_{(l)}} \schemestop. On a égalité des potentiels des deux phases soit $\mu_{Zn(s)}^\circ=\mu_{Zn(l)}^\circ$ à la température de changement d'état. Ainsi, à $T=T_{fus}$, $ \Delta rG^\circ_{1}=2 \mu^\circ_{ZnO(s)} -\mu^\circ_{O_2(g)} -2\mu^\circ_{Zn(s)}$ est égal à  $\Delta rG^\circ_{2}=2 \mu^\circ_{ZnO(s)} -\mu^\circ_{O_2(g)} -2\mu^\circ_{Zn(l)}$. On a donc une continuité de la courbe, bien que la pente, elle, varie.
...
\item Zinc liquide : Si les grandeurs standards sont inconnues, on se sert de la continuité de $\Delta rG^\circ$ lors des changements d'état et on en déduit : 
...
\end{center}
	\subsection{Détermination graphique de la pression de corrosion}
		La \textbf{pression de corrosion} est définie comme la pression de dioxygène à l'équilibre. Pour tous les équilibres considérés, $K^\circ_{i}=\frac{P^\circ}{P_{cor}}$. De plus, tous ces équilibres sont divariants. Comme on impose la pression en zinc (puisqu'il est dans son état standard), le système devient monovariant et le seul facteur d'équilibre est la pression en dioxygène. On trace donc sur le diagramme la courbe $y=\log\frac{P_{O_2}}{P^\circ}$. A l'intersection entre les deux courbes, on a $P_{O_2}=P_{cor}$
		...
		\section{Diagrammes d'\bsc{Ellingham}}
			\subsection{Principe de construction}
				Dans le même système d'axe (T,$\Delta rG^\circ$), on trace $\Delta rG^\circ=f(T)$ pour divers couples (oxydes/métal) et aussi (\chemfig{H_2O}/\chemfig{H_2}), (\chemfig{CO}/\chemfig{C}) et (\chemfig{CO_2}/\chemfig{CO}). Toutes les équations doivent être écrites avec le même nombre stoechiométriques pour \chemfig{O_2}
